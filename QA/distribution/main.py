...
#file_path = '100_dump.fus1.lammpstrj'
...
num_dump = len(dump_list) #一共有多少帧dump的次数
...
# np.savetxt('concentration_array.txt', np.vstack((c_de_array, c_di_array)).T, fmt='%.8f')
# np.savetxt('concentration_mean.txt', np.array([c_de_mean*1661, c_di_mean*1661,c_de_mean/c_di_mean]), fmt='%.8f')
# 
# for i in HIST:
#     dense=np.sum(hist[7:17])
#     dilute=np.sum(hist[0:5]) + np.sum(hist[-5:])
#     c_de=dense/50000
#     c_di=dilute/125000
...
#np.savetxt('concentration_array.txt', np.vstack((c_de_array, c_di_array)).T, fmt='%.4f')
#np.savetxt('concentration_mean.txt', np.array([c_de_mean, c_di_mean]), fmt='%.4f')
